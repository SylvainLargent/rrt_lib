...
double infinity1 = std::numeric_limits<double>::max();
...
//Useful function
        ...
        Node * generate_random_node(){
            if(dist1(gen1) < goal_sample_rate){
            ...
            double rand_x = dist_x(gen1);
            double rand_y = dist_y(gen1);
            double rand_z = dist_z(gen1);
